...
After taking it, the first check the processor will do is check if the batch is expired. If that's the case, we will discard the batch and return an `Expired(executed_functions)` `ExecutionResult` to the Authorization contract. There might be a case that the batch is `NonAtomic` and it's already partially executed, therefore we return the number of functions that were executed before the expiration.
If it's still valid, the processor will execute it in different ways depending if the batch is `Atomic` or `NonAtomic`.
