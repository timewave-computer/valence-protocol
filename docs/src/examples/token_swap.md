...
- **Party A Deposit account**: a [Valence Base account](../components/base_accounts.md) which Party A will deposit their tokens into, to be exchanged with Party B's tokens.
...
- **Party B Withdraw account**: the account from which Party B can withdraw Party A's tokens after the swap has successfully completed. Note: this can be a [Valence Base account](../components/base_accounts.md), but it could also be a regular chain account, or a smart contract.
- **Party B Deposit account**:  a [Valence Base account](../components/base_accounts.md) which Party B will deposit their funds into, to be exchanged with Party A's funds.
...
- **Party A Withdraw account**: the account from which Party A can withdraw Party B's tokens after the swap has successfully completed. Note: this can be a [Valence Base account](../components/base_accounts.md), but it could also be a regular chain account, or a smart contract.
