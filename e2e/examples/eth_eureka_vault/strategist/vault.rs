...
use valence_e2e::utils::solidity_contracts::{MockERC20, ValenceVault};
...
impl EthereumVault for Strategy {
    ...
    async fn calculate_redemption_rate(
        ...
        let eth_wbtc = MockERC20::new(Address::from_str(&self.cfg.ethereum.denoms.wbtc)?, &eth_rp);
        ...
        // 2. query shares in position account and simulate their liquidation for WBTC
        ...
        let (wbtc_amount, ntrn_amount) = self
        ...
        let eth_deposit_wbtc = self
            .eth_client
            .query(eth_wbtc.balanceOf(Address::from_str(&self.cfg.ethereum.accounts.deposit)?))
            .await?
            ._0;

        info!("[calculate_redemption_rate] eth_deposit_wbtc: {eth_deposit_wbtc}");

        let neutron_deposit_acc_wbtc = self
            .neutron_client
            .query_balance(
        ...
        info!("[calculate_redemption_rate] neutron_deposit_acc_wbtc: {neutron_deposit_acc_wbtc}");

        //   4. R = total_shares / total_assets
        let normalized_eth_deposit_acc_balance = Uint128::from_str(&eth_deposit_wbtc.to_string())?;
        info!("[calculate_redemption_rate] eth_deposit_wbtc: {eth_deposit_wbtc}");
        info!("[calculate_redemption_rate] normalized_eth_deposit_acc_balance: {normalized_eth_deposit_acc_balance}");
        let total_assets = neutron_deposit_acc_wbtc
        ...
        info!("[calculate_redemption_rate] total assets: {total_assets}WBTC");
        info!(
        ...
        info!("[calculate_redemption_rate] scaled total assets: {scaled_assets}WBTC");
        info!(
            "[calculate_redemption_rate] total shares normalized u128: {}SHARES",
            normalized_shares.u128()
        );
