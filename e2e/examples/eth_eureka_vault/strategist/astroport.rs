...
pub trait AstroportOps {
    async fn swap_ntrn_into_wbtc(&self) -> Result<(), Box<dyn Error>>;
...
impl AstroportOps for Strategy {
    ...
    async fn exit_position(&self) -> Result<(), Box<dyn Error>> {
        ...
        let rx = self
            .neutron_client
            .execute_wasm(
                &self.cfg.neutron.libraries.astroport_lwer,
    ...
    async fn enter_position(&self) -> Result<(), Box<dyn Error>> {
        let deposit_account_wbtc_bal = self
            .neutron_client
            .query_balance(
        ...
        if deposit_account_wbtc_bal == 0 {
            warn!("Deposit account must have WBTC in order to LP; returning");
            return Ok(());
        } else {
        ...
        let provide_liquidity_msg =
            &valence_library_utils::msg::ExecuteMsg::<_, ()>::ProcessFunction(
                valence_astroport_lper::msg::FunctionMsgs::ProvideSingleSidedLiquidity {
                    asset: self.cfg.neutron.denoms.wbtc.to_string(),
                    limit: None,
                    expected_pool_ratio_range: None,
                },
            );
        let rx = self
            .neutron_client
            .execute_wasm(
                &self.cfg.neutron.libraries.astroport_lper,
    ...
    /// swaps counterparty denom into wbtc
    async fn swap_ntrn_into_wbtc(&self) -> Result<(), Box<dyn Error>> {
        ...
        if withdraw_account_ntrn_bal == 0 {
            warn!("Withdraw account must have NTRN in order to swap into wbtc; returning");
            return Ok(());
        } else {
            info!("swapping {withdraw_account_ntrn_bal}NTRN into wbtc...");
        }
