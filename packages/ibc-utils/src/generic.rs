...
pub fn ibc_send_message(
    ...
    let msg = match denom_to_pfm_map.get(&denom) {
        None => cosmwasm_std::IbcMsg::Transfer {
            channel_id: channel,
            to_address: to,
            amount: coin(amount, denom),
            timeout: get_timeout(&env, timeout_height, timeout_timestamp),
            memo: Some(memo),
        },
        Some(pfm_config) => cosmwasm_std::IbcMsg::Transfer {
...
fn get_timeout(
    ...
    match (timeout_height, timeout_timestamp) {
