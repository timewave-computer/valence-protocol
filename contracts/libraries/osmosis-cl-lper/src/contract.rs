...
pub fn provide_liquidity_custom(
    ...
    let library_submsg = SubMsg::reply_on_success(delegated_input_acc_msgs, REPLY_ID);

    Ok(Response::default().add_submessage(library_submsg))
}
...
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn reply(_deps: DepsMut, _env: Env, msg: Reply) -> Result<Response, LibraryError> {
    match msg.id {
...
fn handle_liquidity_provision_reply_id(result: SubMsgResult) -> Result<Response, LibraryError> {
