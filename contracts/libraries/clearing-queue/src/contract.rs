...
mod functions {
    ...
    use crate::msg::{Config, FunctionMsgs, WithdrawalObligation};
    ...
    pub fn process_function(
        ...
        match msg {
            FunctionMsgs::RegisterObligation(withdrawal_obligation) => {
                try_register_registration(deps, cfg, withdrawal_obligation)
            }
            FunctionMsgs::SettleNextObligation {} => try_settle_next_obligation(deps, cfg),
        }
    }

    fn try_register_registration(
        deps: DepsMut,
        cfg: Config,
        withdraw_obligation: WithdrawalObligation,
    ) -> Result<Response, LibraryError> {
        Ok(Response::new())
    }
