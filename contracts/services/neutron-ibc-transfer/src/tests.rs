...
use valence_service_utils::{
    ...
    testing::{CustomServiceTestSuiteBase, ServiceTestSuite, TestApp},
    ServiceAccountType,
};
...
struct IbcTransferTestSuite {
    #[getset(get)]
    inner: CustomServiceTestSuiteBase<Empty, NeutronQuery, NeutronKeeper>,
    #[getset(get)]
...
impl IbcTransferTestSuite {
    pub fn new(input_balance: Option<(u128, String)>) -> Self {
        let app = AppBuilder::new_custom()
            ...
            .build(no_init);
        let mut inner = CustomServiceTestSuiteBase::new(app);
...
impl ServiceTestSuite<Empty, NeutronQuery, NeutronKeeper> for IbcTransferTestSuite {
...
fn ibc_transfer_fails_for_insufficient_fee_balance() {
    ...
    let cfg: ServiceConfig = suite.ibc_transfer_config(
        ATOM.to_string(),
