    ...
    ($(($variant:ident, $type:ty)),* $(,)?) => {
        ...
        impl OsmosisTypes {
                    ...
                    pub fn reconstruct_response(&self, request: &QueryReconstructionRequest) -> StdResult<Value> {
                        match self {
                            $(
                                OsmosisTypes::$variant(_t) => <$type>::decode_and_reconstruct(request),
                            )*
                        }
                }}
    };
...
impl QueryTypeDefinition for osmosis_std::types::cosmos::bank::v1beta1::QueryBalanceResponse {
    ...
    fn decode_and_reconstruct(request: &QueryReconstructionRequest) -> StdResult<Value> {
        let balances: neutron_sdk::interchain_queries::v047::types::Balances =
            KVReconstruct::reconstruct(&request.icq_result.kv_results).unwrap();

        let balances_str = to_json_string(&balances)?;

        let json_value: Value = serde_json::from_str(&balances_str).unwrap();
