...
use valence_middleware_utils::canonical_types::pools::xyk::{ValenceXykAdapter, ValenceXykPool};
...
impl ValenceXykAdapter for OsmosisXykPool {
    ...
    fn try_from_canonical(canonical: ValenceXykPool) -> Result<Self::External, MiddlewareError> {
        ...
        // unpack the pool id
        let id: u64 = try_unpack_domain_specific_value(ID_KEY, &canonical.domain_specific_fields)?;
        ...
        let pool_params: Option<PoolParams> =
            try_unpack_domain_specific_value(POOL_PARAMS_KEY, &canonical.domain_specific_fields)?;

        // unpack the shares denom and total shares amount before combining them to a proto coin
        let shares_denom: String =
            try_unpack_domain_specific_value(SHARES_DENOM_KEY, &canonical.domain_specific_fields)?;
        ...
        let total_weight: String =
            try_unpack_domain_specific_value(TOTAL_WEIGHT_KEY, &canonical.domain_specific_fields)?;
